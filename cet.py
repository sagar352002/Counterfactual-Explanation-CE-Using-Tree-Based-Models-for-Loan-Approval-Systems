...
class Node():
    ...
    def getRatio(self):        # self.active is the number of samples that satisfy the condition at the current node. If a node is not a leaf node, it means the number of samples that have passed through this node's condition.
        return self.active / self.sample.shape[0]

    def getObjective(self):
        if(self.is_leaf()):  # the number of misclassification at the particular node. for example: CLASSIFY THE FAILING AND PASSING STUDENTS.Leaf1 (<= 5 study hours): 3 misclassifications#Leaf2 (5 < study hours <= 10): 1 misclassification#Leaf3 (10 < study hours <= 15): 2 misclassifications#Leaf4 (> 15 study hours): 0 misclassifications 
...
# stochastic local search algorithm
    def optimizeTree(self):
        ...
        return self.dummy_.left #The optimizeTree method performs a depth-first traversal of the decision tree starting from self.dummy_.left. During traversal, it:

# Updates leaf nodes with new actions if needed.
# Splits non-leaf nodes based on their feature thresholds.
# Adjusts the tree structure by potentially removing nodes with no samples
    ...
    def fit(self, X, max_change_num=4, cost_type='MPS', C=1.0, gamma=1.0, max_leaf_size=-1,
    ...
    def selectEditOperation(self):   #Insert, delete, replace
...
if(__name__ == '__main__'):
    MAX_ITERATION = 50
    _check(dataset='g', model='X', params=(0.06, 1.0), max_iteration=2, lime_approximation=False, verbose=False)
